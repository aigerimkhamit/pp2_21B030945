...
sql = """
SELECT exists(select 1 from second where username=%s)
"""
...
result = cur.fetchone()
if result[0]:
    sql = "select level from second where username = %s"
    cur.execute(sql, (username,))
    num = cur.fetchone()
    level = num[0]
else:
    sql = "insert into second (username, level) values (%s, %s)"
...
screen.fill(OLIVE)
background = pygame.transform.smoothscale(pygame.image.load('back.jpg'), (800, 600))
